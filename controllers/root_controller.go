...
func (u *Root) Index(c web.C, w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logging.SharedInstance().MethodInfo("RootController", "Index", false, c).Infof("login error: %v", err)
		u.About(c, w, r)
		return
	}
	// ログインしている場合はダッシュボードへ
	logging.SharedInstance().MethodInfo("RootController", "Index", false, c).Info("login success")
	...
	if projectID != 0 {
		...
		if err != nil || parentProject.UserID != currentUser.ID {
			logging.SharedInstance().MethodInfo("RootController", "Index", false, c).Warnf("project not found: %v", err)
			NotFound(w, r)
			return
		}
	}
	...
	if err != nil {
		logging.SharedInstance().MethodInfo("RootController", "Index", true, c).Errorf("template error: %v", err)
...
func (u *Root) About(c web.C, w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logging.SharedInstance().MethodInfo("RootController", "About", true, c).Errorf("CSRF error: %v", err)
		InternalServerError(w, r)
		return
	}
	tpl, err := pongo2.DefaultSet.FromFile("about.html.tpl")
	if err != nil {
		logging.SharedInstance().MethodInfo("RootController", "About", true, c).Errorf("template error: %v", err)
		InternalServerError(w, r)
		return
	}
