...
func (u *Lists) Index(c web.C, w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logging.SharedInstance().MethodInfo("ListsController", "Index", false).Infof("login error: %v", err)
	...
	if err != nil || parentProject.UserID != currentUser.ID {
		logging.SharedInstance().MethodInfo("ListsController", "Index", false).Warnf("project not found: %v", err)
		http.Error(w, "project not found", 404)
		return
	}
	...
	logging.SharedInstance().MethodInfo("ListsController", "Index", false).Info("success to get lists")
	return
}
...
func (u *Lists) Create(c web.C, w http.ResponseWriter, r *http.Request) {
	...
	currentUser, err := LoginRequired(r)
	if err != nil {
		logging.SharedInstance().MethodInfo("ListsController", "Create", false).Infof("login error: %v", err)
		http.Error(w, "not logined", 401)
		return
	}
	encoder := json.NewEncoder(w)
	...
	parentProject, err := projectModel.FindProject(projectID)
	if err != nil || parentProject.UserID != currentUser.ID {
		logging.SharedInstance().MethodInfo("ListsController", "Create", false).Warnf("project not found: %v", err)
		http.Error(w, "project not found", 404)
		return
	}
	...
	logging.SharedInstance().MethodInfo("ListsController", "Create", false).Debugf("post new list parameter: %+v", newListForm)
	...
	logging.SharedInstance().MethodInfo("ListsController", "Create", false).Info("success to create list")
	return
}
...
func (u *Lists) Update(c web.C, w http.ResponseWriter, r *http.Request) {
	...
	currentUser, err := LoginRequired(r)
	if err != nil {
		logging.SharedInstance().MethodInfo("ListsController", "Update", false).Infof("login error: %v", err)
		http.Error(w, "not logined", 401)
		return
	}
	encoder := json.NewEncoder(w)
	...
	parentProject, err := projectModel.FindProject(projectID)
	if err != nil || parentProject.UserID != currentUser.ID {
		logging.SharedInstance().MethodInfo("ListsController", "Update", false).Warnf("project not found: %v", err)
		http.Error(w, "project not found", 404)
		return
	}
	...
	logging.SharedInstance().MethodInfo("ListsController", "Update", false).Debugf("post edit list parameter: %+v", editListForm)
	...
	encoder.Encode(jsonList)
	logging.SharedInstance().MethodInfo("ListsController", "Update", false).Info("success to update list")
	return
}
